...
axios.get("http://localhost:8072/FoodiFy/Premium/getPending", { headers: authHeader() })
  .then(data => {
    ...
    Object.keys(X).map((key, index) => {
      ...
      rows.push(createData(X[key].id, X[key].resturant, X[key].purches_date.substring(0,10), X[key].purches_time.substring(10,), X[key].calaries, X[key].fat, X[key].protein, X[key].carbo, X[key].price))
